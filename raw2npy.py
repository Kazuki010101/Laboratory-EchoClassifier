    ...
    def __call__(self, idx, datafile, signals, labels=None):
        if self.spf:
            ...
            if self.is_SHL:
                ...
                filename_last = os.path.basename(datafile).replace('.csv', '')

            # 保存先ディレクトリの作成
            dir_path = os.path.join(self.output_dir, 'train' if idx in self.train_idxs else 'test')
            os.makedirs(dir_path, exist_ok=True)

            # Acc.npy の保存
            filename = os.path.join(dir_path, filename_last + '.npy')
            np.save(filename, signals)

            # Label.npy の保存（ラベルがある場合）
            if labels is not None:
                split_list = datafile.split('/')
                filename_last = split_list[-2]+'_Label'
                label_filename = os.path.join(dir_path, filename_last + '.npy')
...
class ProcessSHL(Process_):  # SHL
    ...
    def data_load(self, datafile, eps=1e-4):
        ...
        # ラベルファイルの読み込み
        label_file = datafile.replace("Acc.txt", "Label.txt")
        if os.path.exists(label_file):
            labels_df = np.loadtxt(label_file)
            labels_df[:, 0] *= 1e-3  # ms → s
            self.label_splits = np.split(labels_df, skip_indexes)
        else:
            self.label_splits = [None] * len(self.splits)
    ...
    def split_window(self):
        ...
        for i, split in enumerate(tqdm(self.splits)):
            index, value = np.split(split, [1], axis=-1)
            label_values = self.label_splits[i][:, 1] if self.label_splits[i] is not None else None
            
            for start in time_range(0, len(index) - 1, step=self.args.window_step_len):
                window = resolution_fn(value, start)
                if window.shape[0] == int(self.args.hz * self.args.window_sec):  # Crucial check!
                    self.windows.append(window)

                    if label_values is not None:
                        label_window = label_values[start:start + self.args.window_len]
                        # Ensure label_window has the same length as the window
                        if len(label_window) == window.shape[0]:
                            self.labels.append(np.median(label_window)) # Use median only if length matches
    ...
    def __call__(self):
        for i, datafile in tqdm(enumerate(self.args.datafiles)):
            self.data_load(datafile)
            self.split_window()
            self.save_file(i, datafile, self.windows, self.labels)
...
def get_args():
    ...
    parser.add_argument('--datafiles', type=str, default='/home/SHL/SHL_2023/train/*/Acc.txt')
    parser.add_argument('--output-dir', type=str, default='./SHL_2023/SHL')
    ...
    parser.set_defaults(
        ...
        window_sec=5.0,
        overlap_sec=0.0,
        datafiles='dataset/SHL_2023/train/*/Acc.txt',
        dataset='SHL',
    )
