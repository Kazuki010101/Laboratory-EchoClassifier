...
class SenvtTeacherAdapter(nn.Module):
    ...
    @torch.no_grad()
    def forward_with_temporal_evidence(self, x, teacher_layer=1, student_patch_size=16):
        """Return logits and exact per-student-patch CLS value contributions."""
        layer = self.core.layers[teacher_layer]
        captured = {}
        pre = layer.attention.register_forward_pre_hook(
            lambda _m, inputs: captured.update(token_input=inputs[0]))
        post = layer.attention.register_forward_hook(
            lambda _m, _i, out: captured.update(attention_output=out[0], weights=out[1]))
        try:
            logits = self.forward(x)
        finally:
            pre.remove(); post.remove()
        token_input = captured["token_input"]
        weights = captured["weights"]
        batch, tokens, dim = token_input.shape
        heads = layer.attention.nheads
        qkv = layer.attention.qkv(token_input).reshape(batch, tokens, 3, heads, dim // heads)
        value = qkv[:, :, 2].permute(0, 2, 1, 3)
        cls_attention = weights[:, :, 0, 1:]
        weighted = cls_attention.unsqueeze(-1) * value[:, :, 1:, :]
        concatenated = weighted.permute(0, 2, 1, 3).reshape(batch, tokens - 1, dim)
        time_evidence = torch.nn.functional.linear(concatenated, layer.attention.proj.weight)
        length = time_evidence.shape[1]
        if length % student_patch_size:
            raise ValueError(f"Teacher length {length} is not divisible by {student_patch_size}")
        patch_evidence = time_evidence.reshape(batch, length // student_patch_size,
                                               student_patch_size, dim).sum(dim=2)
        route = torch.softmax(torch.log(patch_evidence.norm(dim=-1).clamp_min(1e-12)), dim=-1)
        return logits, route, patch_evidence
