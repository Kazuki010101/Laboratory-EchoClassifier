...
from first_distilation import train_with_validation as edtd_train


class EDTDDatasetWrapper(torch.utils.data.Dataset):
...
def get_args_parser():
    ...
    parser.add_argument('--use-edtd', action='store_true', help='Use E-DTD trainer instead of standard KD loop')

    parser.add_argument('--kd-mode', default='e-dtd', choices=['no-kd', 'fixedT', 'dtd', 'e-dtd', 'e-dtd-cons'], help='Knowledge distillation mode for E-DTD trainer')
    return parser
...
def main(args):
    ...
    if args.use_edtd:
        dataset_train = EDTDDatasetWrapper(dataset_train)
        dataset_val = EDTDDatasetWrapper(dataset_val)
    ...
    if args.ThreeAugment and not args.use_edtd:
    ...
    if mixup_active and not args.use_edtd:
        mixup_fn = Mixup1D(
    ...
    if args.student == "MLPMixer":
        ...
        print("Creating model: PatchAttnReservoir")
        model = PatchEchoClassifier.PatchReservoir(
            in_channels=3, patch_size=args.patch_size, stride=args.patch_size,
            reservoir_size=args.reservoir_size, num_classes=args.nb_classes
        )
    elif args.student == "PESAC":
        print("Creating model: PatchSparseAttnReservoir")
    ...
    teacher_model = None
    need_teacher = args.distillation_type != 'none' or args.use_edtd
    if need_teacher:
        print(f"Distillation teacher creation: model={args.model}")
        if args.model == "Transformer":
            ...
            teacher_model = MomentTeacherAdapter(
        ...
        print("Distillation OFF (no teacher)")

        if args.use_edtd:
        if teacher_model is None:
            raise RuntimeError("E-DTD requires a teacher model (set --model and --teacher-path correctly).")
        ...
        # kd-mode に応じて選択
        if args.kd_mode == 'no-kd':
            config = config_no_kd
        elif args.kd_mode == 'fixedT':
            config = config_fixedT
        elif args.kd_mode == 'dtd':
            config = config_dtd
        elif args.kd_mode == 'e-dtd':
            config = config_e_dtd
        elif args.kd_mode == 'e-dtd-cons':
            config = config_e_dtd_cons
        else:
            raise ValueError(f"Unknown kd_mode: {args.kd_mode}")

        save_dir = args.output_dir if args.output_dir != "" else "./results_edtd"
