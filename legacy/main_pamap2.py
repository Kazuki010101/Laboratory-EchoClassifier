...
def get_args_parser():
    ...
    parser.add_argument('--data', default='SHL', choices=['SHL2023','SHL2024','ADL','PAMAP','PAMAP2','REALWORLD','WISDM','CAPTURE','SHL2023_test'])
    parser.add_argument('--pamap2_root', default='dataset/pamap2_100hz_w496_s248_chest16g', type=str)
    parser.add_argument('--pamap2_drop_subj', default=109, type=int)
    parser.add_argument('--fold_test_subj', default=108, type=int)   # LOSO test subject
    parser.add_argument('--fold_val_subj', default=107, type=int)    # validation subject
    parser.add_argument('--val_ratio', default=0.1, type=float)   # trainから抜く割合（ウィンドウ単位）
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        print(f"[VAL] loss={val_loss:.4f} acc1={val_stats['acc1']:.2f}")
    ...
    if data_loader_test is not None and args.output_dir:
        ...
        if best_path.exists():
            ...
            print(f"[TEST@BEST] subj={args.fold_test_subj} loss={test_stats['loss']:.4f} acc1={test_stats['acc1']:.2f}")
    
            if utils.is_main_process():
                with (output_dir / "test_best.json").open("w") as f:
                    json.dump({
