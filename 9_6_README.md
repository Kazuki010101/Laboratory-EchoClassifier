...
Seed 0の結果から提案手法の中心仮説が支持された場合、複数seedによる再現性の確認とアブレーションを実施する。

### 15.1 複数seed

最低限、次の3条件を複数seedで評価する。
...
PRC-KDは全31パッチを処理する基準、TG-Skipは教師に基づいて選択した16パッチのみを処理する条件、TGECは16個の選択パッチと1個の要約トークンを処理する提案条件である。

計算時間に余裕がある場合は、比較対象としてAPS-KDも追加する。
...
各seedでは、データ分割、モデルの初期値、リザバーの初期値、およびrouterの初期値を変更する。  
設定したseedと出力ディレクトリ名の`seedN`が一致していることを、実行前に確認する。

評価値$`x`$について、$`S`$個のseedから得られた値を$`x_1,\ldots,x_S`$とする。  
平均値は次式で求める。
...
Latencyについては、モデル初期値の違いよりも実行環境の変動による影響を受ける可能性がある。  
そのため、すべての条件を同一のハードウェア、ソフトウェア環境、batch size、および測定回数で比較する。

### 15.2 アブレーション

アブレーションでは、TGECを構成する各要素が分類性能と計算効率に与える影響を評価する。  
実験の優先順位は次のとおりである。
...
最初に、TG-SkipとTGECを比較する。

```text
TG-Skip：非選択パッチを破棄
TGEC：非選択パッチを1個の要約トークンへ凝縮
```

両条件では、教師モデル、教師信号を抽出するlayer、選択パッチ数、logit蒸留、およびPRCの構造を共通化する。  
これにより、非選択パッチを破棄する場合と、要約トークンとして保持する場合の差を評価する。

次に、パッチ保持率を変更する。

```text
keep_ratio = 0.25, 0.50, 0.75
```

パッチ数を$N=31$、保持率を$r$とすると、選択パッチ数は次式で求める。

```math
K=\lceil rN\rceil
```

保持率を下げるとPRCの状態更新回数を削減できる一方、個別に保持されるパッチが減少する。  
保持率ごとにaccuracy、macro F1、処理トークン数、MACs、および推論遅延を比較する。

教師信号を抽出するlayerについては、次の条件を比較する。

```text
teacher_layer = 0, 1, 2
```

各layerについて、Step 7で測定したTop-16 mass、正規化エントロピー、およびTGECの分類性能を比較する。  
これにより、重要度分布の偏りと生徒モデルの分類性能の関係を確認する。

損失重みについては、経路蒸留損失の重み$\lambda_{\mathrm{route}}$と、内容蒸留損失の重み$\lambda_{\mathrm{content}}$を個別に変更する。

```text
route loss weight:
lambda_route = 0.0, 0.5, 1.0

content loss weight:
lambda_content = 0.0, 0.5, 1.0
```

$\lambda_{\mathrm{route}}=0$では教師重要度分布を用いたパッチ選択の蒸留を行わない。  
$\lambda_{\mathrm{content}}=0$では、要約トークンを教師側の非選択パッチ表現へ近づける内容蒸留を行わない。

最後に、要約トークンの生成方法を比較する。
...
`meanのみ`では、非選択パッチの平均ベクトルだけを圧縮器へ入力する。  
`mean + RMS`では、平均ベクトルとRMSベクトルを連結して圧縮器へ入力する。  
この比較により、非選択パッチの変動量をRMSとして保持する効果を評価する。

すべての条件を最初から総当たりで実行しない。  
まずTG-SkipとTGECの比較によって情報凝縮の効果を確認し、その後、パッチ保持率、teacher layer、損失重み、および要約方法の順に必要な条件だけを追加する。
